...
tcd_2021_folder <- 'A:/Copernicus/Forest/TreeCoverDensity/TCD2021/TCD2021_10m'

# Data 
lau <- vect(here('f02_data', 'f01_input', 'degree_of_urbanisation', 'f02_DGURBA_2021', 'DGURBA_RG_01M_2021_4258.shp' ))
...
cellcode_names_21 <- paste0(tcd_2021_folder, '/CLMS_HRLVLCC_TCD_S2021_R10m_', cellcode_at, '_03035_V01_R00.tif')
cellcode_names_23 <- paste0(tcd_2021_folder, '/CLMS_HRLVLCC_TCD_S2023_R10m_', cellcode_at, '_03035_V01_R00.tif')


## ---- TCC Indicator ----

# Select Austrian LAU´s
lau_at <- lau %>% filter( CNTR_CODE  == 'AT')

lau_at_3035 <- project(lau_at, 'EPSG:3035') 

lau_at_3035$area_m <- expanse(lau_at_3035, unit='m')
# Create a TCD mosaic
tcd_2021_vrt <- vrt(cellcode_names_21)

## ---- Zonal statistics LAU ----

# WVL 21
lau_wvl_21 <- exact_extract(wvl, 
                            lau_at_3035 %>% st_as_sf(), 
                           c('count', 'frac'), 
                           append_cols = c('GISCO_ID', 'CNTR_CODE', 'DGURBA', 'LAU_ID', 'LAU_NAME', 'area_m'))  
lau_wvl_21 <- lau_wvl_21 %>%  mutate(tcc_pct_wvl21 = frac_1)


# TCD 24
lau_tcd_21 <- exact_extract(tcd_2021_vrt, 
                            lau_at_3035 %>% st_as_sf(), 
                           c('count', 'frac'), 
                           append_cols = c('GISCO_ID', 'CNTR_CODE', 'DGURBA', 'LAU_ID', 'LAU_NAME', 'area_m'))  %>%
  pivot_longer(
    ...
    group_by(GISCO_ID) %>%   
    summarise(
      tcc_pct_tcd21 = sum(canopy_value * fraction, na.rm = TRUE)/ 100, # This presents the sum of fraction per value
      .groups = "drop"
    )
...
lau_zs_at <- lau_zs %>%
  mutate(
    ...
    abs_diff = case_when(
      ...
      TRUE ~ tcc_pct_tcd21 - tcc_pct_wvl21
    ),
    pct_diff = case_when(
      is.na(tcc_pct_wvl21) ~ NA_real_,
      tcc_pct_wvl21 == 0 ~ NA_real_,   
      TRUE ~ ((tcc_pct_tcd21 - tcc_pct_wvl21) / tcc_pct_wvl21) * 100
    )
  ) %>% relocate(DGURBA_lbl, .after=DGURBA) %>% 
  rename(lau_area_m = area_m)

# Export
write_xlsx(lau_zs_at, here('f02_data','f02_processing','zs_lau_at.xlsx'))
